# Sprint 01

기간: 2026.05.26 ~ 2026.06.07

## Goals
1. Java : 고급 3편 인강 및 정리
2. DB : 설계 2편 인강 및 정리
3. 오픽 : 하루 1개씩 토픽 모의고사
4. 책 : 클린 코드
5. 코테 : 하루 1문제

## Progress

| 25 |26 | 27 | 28 | 29 | 30 | 31 | 01 | 02 | 03 | 04 | 05 | 06 | 07 |
|:--:|:-:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| ✅  | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ✅ | ⬜ | ⬜ | ⬜ | ⬜ |

## Daily Log
하루 한 줄. 결과물은 커밋/PR/repo 링크로. (긴 회고는 retrospective.md에)

| 날짜    | 한 일                                                                                                                                                                                                      | 결과물                                                                                                                                |
|-------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| 05.25 | - Java 노션 정리 (동시성 컬렉션, 스레드 풀과 Executor 프레임워크) <br/> - DB 노션 정리 (서브쿼리, UNION, CASE문, 뷰, 인덱스) <br/>- 클린 코드 독서 <br/>- 코테 1문제 <br/>- 오픽 1개 토픽 모의고사 <br/>- AI 개발자 특강                                          | https://github.com/Vryez11/java-play-ground <br/>https://github.com/Vryez11/db-study<br/>https://github.com/Vryez11/java-algorithm |
| 05.26 | - Java 노션 정리 (문자 인코딩, I/O (바이트 스트림, 문자 스트림, 활용, File, Files, Path)) <br/> - DB 노션 정리 (데이터 무결성, 트랜잭션, 저장 프로시저, 함수, 트리거)  <br/>- 클린 코드 독서<br/>- 코테 1문제 <br/>- 서비스 기획 특강                                    | https://github.com/Vryez11/java-play-ground<br/> https://github.com/Vryez11/db-study <br/> https://github.com/Vryez11/java-algorithm                                        |
| 05.27 | - Java 인프런 강의 (네트워크 - 기본 이론, 프로그램 1)<br/>- Java 노션 정리 (네트워크 - 기본 이론, 프로그램 1)<br/>- 클린 코드 독서<br/>- 코테 1문제                                                                                                 | https://github.com/Vryez11/java-play-ground<br/>https://github.com/Vryez11/java-algorithm                                                                                                                                   |
| 05.28 | - Java 인프런 강의 (네트워크 - 프로그램 2)<br/>- Java 노션 정리 (네트워크 - 프로그램 2)<br/>- 코테 1문제 <br/>- AI 기술 교육 강의 (Prompt Engineering 1강, 2강, 3강, 4강)                                                                       |https://github.com/Vryez11/java-play-ground<br/>https://github.com/Vryez11/java-algorithm                                                                                                                                    |
| 05.29 | - AI 기술 교육 강의 (Prompt Engineering 5강, 6강, 7강, 8강, 9강, 10강) <br/>- AI 기술 교육 강의 (Agentic Workflow 1강, 2강)                                                                                                  |                                                                                                                                    |
| 06.01 | - 코테 1 문제 <br/>- 클린 코드 독서                                                                                                                                                                                | https://github.com/Vryez11/java-algorithm                                                                                                                                   |
| 06.03 | - 코테 1 문제 <br/>- Java 인프런 강의 (네트워크 - 채팅 프로그램, HTTP - 기본 이론, 네트워크 - HTTP 서버 만들기) <br/> - Java 노션 정리 (네트워크 - 채팅 프로그램, 네트워크 - HTTP 서버 만들기) <br/> - AI 기술 교육 협업 프로젝트 <br/>- HTTP 기본 이론 노션 정리 <br/>- 클린 코드 독서 | https://github.com/Vryez11/java-algorithm<br/>https://github.com/Vryez11/java-play-ground <br/>https://github.com/sanghyeon1225/asm-team18-ai-study<br/>https://www.notion.so/HTTP-3748e70ba5ff8050ae4dd2bc4a379ccc?source=copy_link                                                                                                                                  |

## Results
스프린트 종료시, 핵심 산출물만 요약.

## Learned
- 

## Related Links
- GitHub:
- Notion (학습 노트):

> 회고는 [retrospective.md](./retrospective.md) 참고